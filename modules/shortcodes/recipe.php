...
class Jetpack_Recipes {
	...
	function add_scripts() {
		...
		foreach ( $GLOBALS['posts'] as $p ) {
			if ( false !== strpos( $p->post_content, '[recipe' ) ) {
		...
		wp_enqueue_style( 'jetpack-recipes-style', plugins_url( 'recipes.css', __FILE__ ), array(), '20130919' );
		wp_add_inline_style( 'jetpack-recipes-style', self::themecolor_styles() ); // add $themecolors-defined styles

		wp_enqueue_script( 'jetpack-recipes-printthis', plugins_url( 'printthis.js', __FILE__ ), array( 'jquery' ), '20131230' );
		wp_enqueue_script( 'jetpack-recipes-js', plugins_url( 'recipes.js', __FILE__ ), array( 'jquery', 'jetpack-recipes-printthis' ), '20131230' );

		$title_var     = wp_title( '|', false, 'right' );
		$print_css_var = plugins_url( 'recipes-print.css', __FILE__ );
